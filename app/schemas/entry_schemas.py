    ...
    def validate_numbers(self, value):
        if not value:
            raise ValidationError(
                "Les numéros classiques sont requis")

        number_list = [
            int(num) for num in value.split(',')]

        if len(number_list) < 5:
            raise ValidationError(
                "Il manque des numéros (minimum 5 requis)")

        if len(set(number_list)) != len(
                number_list):
            raise ValidationError(
                "Les numéros doivent être différents")
        if list(filter(lambda x: not (
                1 <= x <= 49), number_list)):
            raise ValidationError(
                "Les numéros doivent être entre 1 et 49")

    @validates('lucky_numbers')
    def validate_numbers_lucky(
            self, value):
        if not value:
            raise ValidationError(
                "Les numéros chanceux sont requis")
        ...
        if len(set(lucky_number_list)) != len(
                lucky_number_list):
            raise ValidationError(
                "Les numéros chanceux doivent être différents")

        if list(filter(lambda x: not (
                1 <= x <= 0), lucky_number_list)):
            raise ValidationError(
                "Les numéros doivent être entre 1 et 9")
